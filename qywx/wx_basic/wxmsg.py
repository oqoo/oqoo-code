...
class EventMessage(WechatMessage):
    def __init__(self, message):
        ...
        try:
            ...
            if self.type == 'subscribe' or self.type == 'enter_agent':
                self.key = message.pop('EventKey', None)
            elif self.type == 'scancode_push' or self.type == 'scancode_waitmsg':
                self.key = message.pop('EventKey', None)
                self.scancodeinfo = message.pop('ScanCodeInfo', None)
            elif self.type == 'pic_sysphoto' or self.type == 'pic_photo_or_album' or self.type == 'pic_weixin':
                self.key = message.pop('EventKey', None)
                self.sendpicsinfo = message.pop('SendPicsInfo', None)
                ...
                self.key = message.pop('EventKey', None)
                self.sendlocationinfo = float(message.pop('SendLocationInfo'))
            elif self.type == 'batch_job_result':
                self.batchjob = message.pop('BatchJob', None)
